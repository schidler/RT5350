...
static void build_console(void)
{
        ...
        r = sys_mknod("/dev/console", 0662 | S_IFCHR, mknoddev(5,1));
}
#endif

static void mount_default_parts(void);

static void mount_default_parts(){
	unsigned long flags;
	...
	flags&=~MS_SYNCHRONOUS;
...
#endif
#ifdef CONFIG_TMPFS
        if (sys_mount("tmpfs", "/dev", "tmpfs", flags, NULL) < 0)
        ...
        if (sys_mount("tmpfs", "/tmp", "tmpfs", flags, NULL) < 0)
            printk("mount /tmp file system fail!\n");
            else
            printk("mount /tmp file system ok!\n");

        if (sys_mount("tmpfs", "/etc", "tmpfs", flags, NULL) < 0)
...
static noinline void __init kernel_init_freeable(void)
{
	...
	if ((s = (int)sys_open((const char __user *) "/dev/console", O_RDWR, 0)) < 0)
		printk(KERN_WARNING "Please be patient, while tWRT loads (cannot open console (error %d))...\n", (int)s);
