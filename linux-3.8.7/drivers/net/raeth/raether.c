...
inline int ei_start_xmit(struct sk_buff* skb, struct net_device *dev, int gmac_no)
{
	...
	unsigned int txd_info4, i;
	unsigned long flags;
#if defined (CONFIG_RAETH_SG_DMA_TX)
	unsigned int nr_frags, txd_info2;
...
#if defined (CONFIG_RAETH_SG_DMA_TX)
	if (nr_frags) {
		tx_ring->txd_info1_u32 = dma_map_single(NULL, skb->data, skb->len - skb->data_len, DMA_TO_DEVICE);
...
int ei_open(struct net_device *dev)
{
	...
	*((volatile u32 *)(ESW_IMR)) &= ~(ESW_INT_ALL);
	...
	err = request_irq(OPENWRT_IRQ_ESW, esw_interrupt, IRQF_DISABLED, "Ralink_ESW", dev);
	if (err) {
