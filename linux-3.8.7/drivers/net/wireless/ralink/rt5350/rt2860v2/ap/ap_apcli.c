	...
	IN UCHAR ifIndex)
{
	...
	do
	{
		...
		//printk(KERN_INFO "%d: %s: pAd->ApCliMlmeAux.Bssid: %02x:%02x:%02x:%02x:%02x:%02x\n", __LINE__, __func__, 
		...
		if (pMacEntry)
		{
			...
			//printk(KERN_INFO "%d: %s: pAd->ApCliMlmeAux.Bssid: %02x:%02x:%02x:%02x:%02x:%02x\n", __LINE__, __func__, pAd->ApCliMlmeAux.Bssid[0], pAd->ApCliMlmeAux.Bssid[1], pAd->ApCliMlmeAux.Bssid[2], 
			//	pAd->ApCliMlmeAux.Bssid[3], pAd->ApCliMlmeAux.Bssid[4], pAd->ApCliMlmeAux.Bssid[5]);
			...
			//printk(KERN_INFO "%s:%d %s: ssid: %s\n", __FILE__, __LINE__, __func__, pApCliEntry->Ssid);
	...
	RtmpDrvSendWirelessEventApCli(pAd, ifIndex, pMacEntry, APCLI_LINK_UP);
	return result;
}
	...
	IN UCHAR ifIndex)
{
    ...
    RtmpDrvSendWirelessEventApCli(pAd, ifIndex, NULL, APCLI_LINK_DOWN); 	
	pApCliEntry = &pAd->ApCfg.ApCliTab[ifIndex];
...
/*	RTMP_OS_NETDEV_OP_HOOK	netDevOpHook; */
	...
	/* init */
	printk(KERN_INFO "Ralink APCli_Init()\n");
	...
	for(apcli_index = 0; apcli_index < MAX_APCLI_NUM; apcli_index++)
	{
		...
		printk(KERN_INFO "%s: OpMode is %s.\n", 
			__func__, pAd->OpMode == OPMODE_AP?"AP":pAd->OpMode == OPMODE_STA?"STA":"Unknown");
