...
/*		SendSingalToDaemon(SIGUSR2, pObj->IappPid); */
		...
		RtmpDrvSendWirelessEventExit(pAd, IW_ASSOC_EVENT_FLAG, pEntry->Addr, 0, 0, pEntry);
    ...
    IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Elem->Wcid < MAX_LEN_OF_MAC_TABLE)
    {
		...
		RtmpDrvSendWirelessEventExit(pAd, IW_DEAUTH_EVENT_FLAG, Addr2, 0, 0, pEntry);
	...
	IN USHORT Reason)
{
	...
	if (Aid < MAX_LEN_OF_MAC_TABLE)
	{
        ...
        RtmpDrvSendWirelessEventExit(pAd, IW_DEAUTH_EVENT_FLAG, pStaAddr, 0, 0, pEntry); 
