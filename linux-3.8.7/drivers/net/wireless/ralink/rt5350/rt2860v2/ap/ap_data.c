 ...
 ***************************************************************************
	...
	Jan Lee	    01-20-2006		modify for 2860.  
*/
#include "rt_config.h"

#ifndef IS_MULTICAST_MAC_ADDR
...
#ifdef RBUS_SUPPORT
				/*
					for WMM 4.2.21 T10 case, we can tune the value to pass the case
					the value can not more than mac 1300 txop value	
				*/
				if(pAd->Antenna.field.TxPath == 1)
				{
					AC_TXOP_CSR0_STRUC csr0;

					RTMP_IO_READ32(pAd, WMM_TXOP0_CFG, &csr0.word);
					csr0.field.Ac0Txop = 0x20;
...
if ((pHeader->FC.ToDs==1)&&(pHeader->FC.FrDs==0)) {
...
list_for_each(list_pos,&pAd->RestrictedMac.list) {
	...
	if ( memcmp(temp->Addr,pHeader->Addr2,6) == 0 ) {
    		printk("Received packet from blocked user! Dropping..\n");
		RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
		return 0;
	    }
	}
	...
	IN	ULONG			FromWhichBSSID)
{
	...
	if (pHeader802_3[0] & 0x01) 
	{
		...
		*/
		if (
#ifdef P2P_SUPPORT //yiwei merge carella's patch to pass P2P test plan  6.1.12
