	...
	for (i = 0; i< MAX_LEN_OF_MULTICAST_FILTER_TABLE; i++)
	{
		...
		if (pMulticastFilterTable->Content[i].Valid == TRUE)
		{
			...
			if(IgmpMemberCnt(&pEntry->MemberList) == 0){
				printk(KERN_INFO "%s: %02x:%02x:%02x:%02x:%02x:%02x has no members, removing ...\n",
				__func__, PRINT_MAC(pEntry->Addr));
				memset(mac_addr, 0, 20);
				sprintf(mac_addr, "%02x:%02x:%02x:%02x:%02x:%02x", PRINT_MAC(pEntry->Addr));
				Set_IgmpSn_DelEntry_Proc(pAd, mac_addr);
			}
	...
	IN MulticastFilterEntryType type)
{
	...
	if (pMulticastFilterTable->Size >= MAX_LEN_OF_MULTICAST_FILTER_TABLE)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s: cleaning ...\n", __func__));
		IGMPTableDeleteEmpty(pAd);
		return FALSE;
	}
