	...
	IN	PSTRING			mac_addr)
{
	...
	#endif
	pAd->PermanentAddress[0] = 0x66;//(UCHAR)(Addr01 & 0xff);
	pAd->PermanentAddress[1] = 0x99;//(UCHAR)(Addr01 >> 8);
	pAd->PermanentAddress[2] = RandomByte(pAd);//(UCHAR)(Addr23 & 0xff);
	pAd->PermanentAddress[3] = RandomByte(pAd);//(UCHAR)(Addr23 >> 8);
	pAd->PermanentAddress[4] = RandomByte(pAd);//(UCHAR)(Addr45 & 0xff);
	pAd->PermanentAddress[5] = f % 2 !=0?(UCHAR)(Addr45 >> 8):f;
	
	printk(KERN_INFO "BSSID: %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pAd->PermanentAddress));

	/*more conveninet to test mbssid, so ap's bssid &0xf1*/
	if (pAd->PermanentAddress[0] == 0xff)
		pAd->PermanentAddress[0] = RandomByte(pAd)&0xf8;
			
	//printk(KERN_INFO "E2PROM MAC: =%02x:%02x:%02x:%02x:%02x:%02x\n",
					//			PRINT_MAC(pAd->PermanentAddress));
			 ...
			 (strcmp(mac_addr, "00:00:00:00:00:00") != 0))
	{
		...
		//printk(KERN_INFO "Use the MAC address what is assigned from Moudle Parameter. \n");
	}
	else
	{
		COPY_MAC_ADDR(pAd->CurrentAddress, pAd->PermanentAddress);
		//printk(KERN_INFO "Use the MAC address what is assigned from EEPROM. \n");
	}
