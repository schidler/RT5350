...
Arguments:
    ...
    ==========================================================================
*/
PSTRING GetEncryptTypeForioctl(CHAR enc)
{
    if(enc == Ndis802_11WEPDisabled)
        return "NONE";
    if(enc == Ndis802_11WEPEnabled)
...
PSTRING GetAuthModeForIoctl(CHAR auth)
{
    if(auth == Ndis802_11AuthModeOpen)
    	return "OPEN";
    if(auth == Ndis802_11AuthModeShared)
	...
	IN  UINT32		MsgLen)
{
		...
		(pBss->AuthMode <= Ndis802_11AuthModeWPA1PSKWPA2PSK))
	{
		...
		sprintf(SecurityStr, "%s/%s", GetAuthModeForIoctl((CHAR)ap_auth_mode), GetEncryptTypeForioctl((CHAR)ap_cipher));		
	}			
	else
	{
		ap_auth_mode = pBss->AuthMode;
	...
	sprintf(msg+strlen(msg), "%-23s", SecurityStr);
		...
		if (wireless_mode == Ndis802_11FH ||
			...
			sprintf(Wmode,"%s", "11a/n");
		else if (wireless_mode == Ndis802_11OFDM24)
			sprintf(Wmode,"%s", "11b/g");
		else if (wireless_mode == Ndis802_11OFDM24_N)
			sprintf(Wmode,"%s", "11b/g/n");
		else
			sprintf(Wmode,"%s", "unknow");
		
		sprintf(msg+strlen(msg),"%-7s", Wmode);
     ...
     //   ssid#bssid#channel#signal#mode#sec#type
		sprintf(msg+S, "%s#%02x:%02x:%02x:%02x:%02x:%02x#%d#%d#%s#%s#%s\n",
			Ssid, 
			...
			Wmode,
			SecurityStr,
			pBss->BssType == BSS_ADHOC?"AdHoc":"Infra"
		);
	return;
}
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	//printk(KERN_INFO "%s: pAdapter->ScanTab.BssNr: %d\n", __func__, pAdapter->ScanTab.BssNr);
	for(i=0; i<pAdapter->ScanTab.BssNr ;i++)
	{
		pBss = &pAdapter->ScanTab.BssEntry[i];
		
		if( pBss->Channel==0){
		//	printk(KERN_INFO "%s: pBss->Channel==0\n", __func__);
			break;
		}
...
#define	MAC_LINE_LEN	(14+4+4+10+10+10+12+12+12)	/* Addr+aid+psm+datatime+rxbyte+txbyte+current tx rate+last tx rate*/
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	sprintf(msg,"%s","\n");
	sprintf(msg+strlen(msg),"%-18s%-4s%-8s%-9s%-4s%-10s%-10s%-10s%-10s\n",
		"MAC", "AID", "Signal", "PhyMode", "BW", "TxPackets", "RxPackets", "TxBytes", "RxBytes");
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
		{
			...
			sprintf(msg+strlen(msg),"%-2d", (int)pEntry->Aid);
			sprintf(msg+strlen(msg),"%-4d",  pEntry->RssiSample.AvgRssi0);
			sprintf(msg+strlen(msg),"%-6s",   GetPhyMode(pEntry->HTPhyMode.field.MODE));
			sprintf(msg+strlen(msg),"%-4s",   GetBW(pEntry->HTPhyMode.field.BW));
#ifdef CONFIG_AP_SUPPORT
			sprintf(msg+strlen(msg),"%-12lld", pEntry->TxPackets.QuadPart);
			sprintf(msg+strlen(msg),"%-12lld", pEntry->RxPackets.QuadPart);
			sprintf(msg+strlen(msg),"%-12lld", pEntry->TxBytes);
			sprintf(msg+strlen(msg),"%-12ld\n", (ULONG)pEntry->RxBytes);
#else
