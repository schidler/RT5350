	...
	IN	CHAR					Rssi)
{
	...
	if (pAd->CommonCfg.bWirelessEvent == FALSE){
		printk(KERN_INFO "%s: early exit. bWirelessEvent == FALSE\n", __func__);
		return;
	}
	...
	#if 0
	printk(KERN_INFO "%s: stage 2, event: %0x02x, type: %0x04x, msg: %s\n",
		__func__, event, type, 
		(event >= event_table_len)?0xFF:pWirelessSysEventText[event]);
	#endif
...
VOID RtmpDrvSendWirelessEventExit(
	IN	VOID					*pAdSrc,
	IN	USHORT					Event_flag,
	IN	PUCHAR 					pAddr,
	IN  UCHAR					BssIdx,
...
/*	if((pBuf = kmalloc(IW_CUSTOM_MAX_LEN, GFP_ATOMIC)) != NULL) */
	...
	if(pBuf != NULL)
	{
		...
		if (pAddr == NULL || pEntry == NULL){
			printk(KERN_INFO "%s: null pointer in argv.\n", __func__);
			os_free_mem(NULL, pBuf);
			return;
		}
		if(Event_flag == IW_ASSOC_EVENT_FLAG){
			pBufPtr += sprintf(pBufPtr, "(RT2860) STA(%02x:%02x:%02x:%02x:%02x:%02x) had associated successfully (Signal: %d, Mode: %s, BW: %s)", 
							PRINT_MAC(pAddr), 			 /* MAC */
							pEntry->RssiSample.AvgRssi0, /* Signal */
							GetPhyMode(pEntry->HTPhyMode.field.MODE),
							GetBW(pEntry->HTPhyMode.field.BW));
							pEntry->isBlocked=0;
//							printk("User added and unblocked!\n");
		}
		else {
#ifdef CONFIG_AP_SUPPORT
		total = (pEntry->TxBytes + pEntry->RxBytes);
#else
		total = 0;
#endif
		pBufPtr += sprintf(pBufPtr, "(RT2860) STA(%02x:%02x:%02x:%02x:%02x:%02x) had deauthenticated (Total: bytes: %lld, packets: %ld)", 
			PRINT_MAC(pAddr), /* MAC */
#ifdef CONFIG_AP_SUPPORT
			total,	/* Total bytes */
			(pEntry->TxPackets.QuadPart + pEntry->RxPackets.QuadPart) /* Total packets */
#else
			total, 0
#endif
		);
...
VOID RtmpDrvSendWirelessEventApCli(
	IN  VOID *pAdSrc,
	IN  UCHAR ifIndex,
	...
	IN  UCHAR State)
{
	...
	if(pBuf != NULL)
	{
		...
		switch(State){
				case APCLI_LINK_UP:
					pBufPtr += sprintf(pBufPtr, "(RT2860) APCLI: link UP (apcli%d): SSID: %s, BSSID: %02x:%02x:%02x:%02x:%02x:%02x, Auth mode: %s, Encrypt type: %s, Rate: %d Mbps",
						ifIndex,
						pAd->ApCliMlmeAux.Ssid,
						pAd->ApCliMlmeAux.Bssid[0], pAd->ApCliMlmeAux.Bssid[1], pAd->ApCliMlmeAux.Bssid[2],
						pAd->ApCliMlmeAux.Bssid[3], pAd->ApCliMlmeAux.Bssid[4], pAd->ApCliMlmeAux.Bssid[5],
						GetAuthMode(pAd->ApCfg.ApCliTab[ifIndex].AuthMode),
						GetEncryptType(pAd->ApCfg.ApCliTab[ifIndex].WepStatus),
						RateIdToMbps[pMacEntry->MaxSupportedRate]);
					break;
					
				case APCLI_LINK_DOWN:
					pBufPtr += sprintf(pBufPtr, "(RT2860) APCLI: link DOWN (apcli%d)", ifIndex);
					break;
					
				default:
					pBufPtr += sprintf(pBufPtr, "(RT2860) APCLI: Unknown event (apcli%d)", ifIndex);
					break;
		}
		pBufPtr[pBufPtr - pBuf] = '\0';
		BufLen = pBufPtr - pBuf;
	    RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, IW_ASSOC_EVENT_FLAG, NULL, (PUCHAR)pBuf, BufLen);
		printk(KERN_INFO "Observer: %s\n", pBuf);
		os_free_mem(NULL, pBuf);
	}
	else
