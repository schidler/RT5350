...
#define trace_call      printk(KERN_INFO "%s:%d -->\n", __func__, __LINE__)

/* From arch/mips/mm/dma_default.c */

static void *ra_dma_alloc_coherent(struct device *dev, size_t size,
...
static void ra_dma_free_coherent(struct device *dev, size_t size, void *vaddr,
...
/*	POS_COOKIE pObj = (POS_COOKIE)pAd->OS_Cookie; */
    ...
    DmaAddr = (dma_addr_t)(*PhysicalAddress);

	//*VirtualAddress = (PVOID)pci_alloc_consistent(pPciDev, sizeof(char)*Length, &DmaAddr);
    *VirtualAddress = ra_dma_alloc_coherent(pPciDev == NULL ? NULL : &pPciDev->dev, sizeof(char)*Length, &DmaAddr, GFP_ATOMIC);
...
/*	POS_COOKIE pObj = (POS_COOKIE)pAd->OS_Cookie; */
	...
	ra_dma_free_coherent(pPciDev == NULL ? NULL : &pPciDev->dev, Length, VirtualAddress, DmaAddr);
}
...
/*#else */
		*PhysicalAddress = PCI_MAP_SINGLE_DEV(NULL, *VirtualAddress, Length,  -1, RTMP_PCI_DMA_FROMDEVICE);
/*#endif */
	} else {
...
/*	pObj = (POS_COOKIE)pAd->OS_Cookie; */
	...
	{
		return (ra_dma_addr_t)pci_map_single(NULL, ptr, size, direction);
	}
...
/*	pObj = (POS_COOKIE)pAd->OS_Cookie; */
	...
	if (size > 0)
		pci_unmap_single(NULL, DmaAddr, size, direction);
